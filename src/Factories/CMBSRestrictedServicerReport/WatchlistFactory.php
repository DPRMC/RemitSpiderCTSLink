...
class WatchlistFactory {
    ...
    public function parse( array $rows ): array {
        $this->_setCleanHeaders( $rows );
        dump( $this->cleanHeaders );
...
//        dump($rows);
    ...
    protected function _setCleanHeaders( array $allRows ): void {
        $headerRow = [];
    ...
    protected function _getRowsToBeParsed( array $allRows ): array {
        ...
        dump($allRows[$this->headerRowIndex]);
        dump($this->cleanHeaders);
        dump($allRows[$firstRowOfDataIndex]);

        dump($this->headerRowIndex);
        dump($firstRowOfDataIndex);

        dd('done');

        for ( $i = $this->headerRowIndex; $i < count( $allRows ); $i++ ):
    ...
    protected function _parseRows( array $allRows ): array {
